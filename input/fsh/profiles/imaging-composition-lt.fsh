...
* ^url = $imaging-composition-lt-url
...
* attester[legalAuthenticator].party only Reference($practitioner-role-lt-url)
...
* section[imagingstudy].entry[imagingstudy] only Reference($imaging-study-lt-url)
...
* section[order].entry[order] only Reference($service-request-lt-url)
...
* section[procedure].entry[procedure] only Reference($procedure-lt-url)
...
* section[comparison].entry[comparedstudy] only Reference($imaging-study-lt-url or $ImagingSelectionEuImagingUrl)
...
* section[impression].entry[impression] only Reference($condition-lt-url)
